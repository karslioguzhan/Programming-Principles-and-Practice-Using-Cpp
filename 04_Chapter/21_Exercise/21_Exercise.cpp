// 21_Exercise.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
std::stringstream searchName(std::vector<std::string> names, std::vector<int> ages)
{
    ...
    std::cout << "Please enter a name or age to get the corresponding value: ";
    std::string input;
    std::cin >> input;
    for (int numElement{ 0 }; numElement < names.size(); ++numElement)
    {
        if (names.at(numElement) == input)
        {
            resultSearch << "The person named " << input << " is " << ages.at(numElement) << " years old!\n";
            return resultSearch;
        }
        if (std::to_string(ages.at(numElement)) == input)
        {
            resultSearch << "The person with the age of " << input << " is " << names.at(numElement) << "!\n";
            return resultSearch;
        }

    }
    resultSearch << "name or age not found!\n";
    return resultSearch;
}
